...
const ProductCard = ({ product, className = '', minimal = false }: ProductCardProps) => {
  const { activeBanner } = useAdminBanner()
  
  const wrapperClass = minimal
    ? `bg-gradient-black overflow-hidden border border-brand-gray rounded-none ${className}`
    : `group bg-gradient-black border border-brand-gray/20 rounded-lg overflow-hidden ${activeBanner ? 'admin-dynamic-border-hover' : 'hover:border-brand-emerald/50'} product-card-hover transition-all duration-300 ${className}`

  const imageClass = minimal
  ...
  return (
    <div className={wrapperClass}>
      ...
      {/* Product Details */}
      <div className={`p-4 sm:p-6 ${minimal ? 'text-center' : ''}`}>
